      ...
      func.onTrade(name).then(res => {
        if (res.data.code == 1) {
          if (res.data.data.is_empty == 0) {
            ...
            wx.redirectTo({
              url: '/package/component/pages/intitle/check/check?name=' + name + '&num=' + num + '&servername=' + servername,
            })
          } else {
            getApp().globalData.tradeArry = res.data.data.data;
            wx.navigateTo({
              url: '/package/component/pages/intitle/choose/choose?name=' + name + '&num=' + num + '&servername=' + servername,
            })
          }
        }
      }).catch(err => { })
