      ...
      func.onTrade(text).then(res => {
        if (res.data.code == 1) {
          if (res.data.data.is_empty == 0) {
            ...
            wx.redirectTo({
              url: '/package/component/pages/index/check_history/check_history?text=' + text + '&key=' + key,
            })
          } else {
            getApp().globalData.tradeArry = res.data.data.data;
            wx.navigateTo({
              url: '/package/component/pages/index/choose_history/choose_history?text=' + text + '&key=' + key,
            })
          }
        }
      }).catch(err => {})
