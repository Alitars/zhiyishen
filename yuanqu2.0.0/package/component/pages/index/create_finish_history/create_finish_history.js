...
Page({
  ...
  onBack() {
    ...
    for (var i = 0; i < pages.length; i++) {
      if (pages[i].route == 'package/component/pages/index/brand_name/brand_name') {
        wx.navigateBack({
          delta: i,
        });
        break;
  ...
  onMore() {
    if (this.data.pass) {
      ...
      NetworkRequest({
        ...
        var historyArryIcon = getApp().globalData.historyArryIcon;
        if (code == 1) {
          for (var i = 0; i < list.length; i++) {
            list[i].fir = historyArryIcon;
            var a = index + i;
            this.setData({
              ["imgUrls[" + a + "]"]: list[i]
            });
  ...
  onDt(e) {
    ...
    wx.navigateTo({
      url: '/package/component/pages/index/createDt/createDt?index=' + index + '&s=' + n + '&key=' + this.data.key + '&text=' + this.data.text,
    })
  },
