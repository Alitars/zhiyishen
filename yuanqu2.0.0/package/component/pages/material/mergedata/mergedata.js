...
Page({
  data: {
    ...
    img_business: applyType.img_business,
  ...
  onPreview() {
    if (this.data.imgUrlsTwo) {
      preview(this.data.icon_Url + this.data.imgUrlsTwo, [this.data.icon_Url + this.data.imgUrlsTwo])
    } else {
  ...
  onUpload() {
    ...
    var checkFrou = this.data.checkFrou;
    if (this.data.imgUrlsTwo) {
      var img = this.data.imgUrlsTwo;
    } else {
      var img = this.data.arry.brand_update.brand_img
    };
    ...
    if (brand_img == '' || apply_id.length == 0) {
      Toast('请上传委托书！');
      return;
    };
    NetworkRequest({
  ...
  onChoose() {
    ...
    if (order.length > 0) {
      wx.navigateTo({
        url: '/package/component/pages/material/mergedata/synchro_order/synchro_order',
      })
    } else {
  ...
  onClick() {
    ...
    if (arry) {
      wx.navigateTo({
        url: '/package/component/pages/material/mergedata/dt/dt?order_id=' + arry.order.order_id + "&two_cate=" + arry.brand_update.two_cate + "&three_cate=" + arry.brand_update.three_cate + "&open=" + false,
      })
    } else {
