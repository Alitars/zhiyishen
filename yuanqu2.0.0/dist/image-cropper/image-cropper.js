...
Component({
  ...
  data: {
    ...
    safeHeight: getApp().globalData.systemInfo.statusBarHeight,
    width: 300, //宽度
    height: 300, //高度
    max_width: 400,
    max_height: 400,
  ...
  methods: {
    ...
    rotate() {
      //在用户旋转的基础上旋转90°
      this.setAngle(this.data.angle += 90);
    },
    //end
    end(e) {
    ...
    setCutCenter() {
      ...
      //顺序不能变
      console.log(this.data._img_top, this.data.cut_top, cut_top, '1');
      console.log((this.data._img_top - this.data.cut_top + cut_top) - this.data.img_height / 2);
      if (this.data.width == this.data.img_width) {
        var isCut = true;
      } else {
        var isCut = false;
      };
      this.setData({
        _img_top: isCut ? this.data.info.windowHeight / 2 : this.data._img_top - this.data.cut_top + cut_top,
        cut_top: cut_top, //截取的框上边距
        _img_left: isCut ? this.data.info.windowWidth / 2 : this.data._img_left - this.data.cut_left + cut_left,
        cut_left: cut_left, //截取的框左边距
      });
      console.log(this.data._img_top, this.data.cut_top, cut_top, '2');
      console.log(this.data.info.windowHeight / 2)
    },
    _setCutCenter() {
      let cut_top = (this.data.info.windowHeight - this.data.height) * 0.5;
      let cut_left = (this.data.info.windowWidth - this.data.width) * 0.5;
    ...
    imgReset() {
      ...
      // console.log(this.data._img_top,this.data.img_height);
    },
    /**
     * 加载（更换）图片
     */
