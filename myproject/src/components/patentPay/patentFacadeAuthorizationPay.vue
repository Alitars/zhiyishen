<template>
  <div class="patentFacadeAuthorizationPay">
    <Top></Top>
    <Fix></Fix>
    <div class="patentFacadeAuthorizationPay-box">
      <!-- top -->
      <div class="patentFacadeAuthorizationPay-box-top">
       外观设计专利申请受理缴费
      </div>
      <!-- 受理通知书 -->
      <div class="patentFacadeAuthorizationPay-box-one">
        <div class="one-left">
      ...
      <div class="patentFacadeAuthorizationPay-box-one">
        ...
        <div class="one-right">
          <p>缴费通知书</p>
          <span><a :href="GLOBAL.base_url+this.see2" target="_blank" >预览</a></span>
          <span><a :href="GLOBAL.base_url+'/api/patent/downloadFile' + this.see2">下载</a></span>
        </div>
      </div>
      <!-- 费用明细 -->
      <div class="patentFacadeAuthorizationPay-box-detail">
        <p>费用明细：</p>
        <span>授权费：￥{{auth_fee}}</span>

      </div>
      <!--  选择  -->
      <div class="patentFacadeAuthorizationPay-box-select">
        <p>是否代缴官费：</p>
      ...
      <!-- 支付方式 -->
      <div class="patentFacadeAuthorizationPay-box-way" v-show="switch1">
        <p>支付方式：</p>
        <div :class="active">
      ...
      <!-- 总金额 -->
      <div class="patentFacadeAuthorizationPay-box-money" v-show="switch1">
        <p>总计金额:</p>
        <p>RMB <span>{{auth_fee}}</span></p>
      </div>
      <!-- 提示 -->
      <div class="patentFacadeAuthorizationPay-box-prompt" v-show="switch2">
      ...
      <!-- 提交&&取消 -->
      <div class="patentFacadeAuthorizationPay-box-submit" >
...
<script>
  ...
  export default {
  name:'patentFacadeAuthorizationPay',
  components:{
    Top,
    Foot,
    Fix
  ...
  methods:{
    ...
    // 获取数据
    getData(){
      this.$http.post(this.GLOBAL.base_url+'/api/patent/getRegisterInfo',{
        user_id:this.$cookie.getCookie('u_id'),
        id:this.$route.query.id
      }).then( res => {
        // console.log(res)
        if(res.data.code == '1') {
          this.auth_fee = res.data.data.auth_fee
          this.see1  = res.data.data.regist_notice
          this.see2 = res.data.data.license_notice
        }
      })
    },
     // 选择代理缴费
    chargeAgency() {
      ...
      if (this.find == '1' && this.active == 'company') {
          ...
          this.$http.post(this.GLOBAL.base_url+'/api/patent/payRegisterInfo',{
            user_id:this.$cookie.getCookie('u_id'),
            id:this.$route.query.id,
            type:4,
...
<style scoped>
  .patentFacadeAuthorizationPay-box {
    width: 1302px;
  ...
  /* top */
  .patentFacadeAuthorizationPay-box-top {
    height: 110px;
    line-height: 110px;
  ...
  /* 说明书&&委托书 */
  .patentFacadeAuthorizationPay-box-one {
    height: 230px;
  ...
  /* 费用明细 */
  .patentFacadeAuthorizationPay-box-detail {
    border-top: 1px solid #d8d8d8;
    padding-bottom: 32px;
    border-bottom: 1px solid #d8d8d8;
  }
  .patentFacadeAuthorizationPay-box-detail > p {
  ...
  .patentFacadeAuthorizationPay-box-detail > span {
    font-family: "PingFangSC";
    font-size: 18px;
    margin-right: 96px;
  }
  /* 选择 */
  .patentFacadeAuthorizationPay-box-select {
    margin: 36px 0 36px 0px;
    border-bottom: 1px solid #d8d8d8;
    padding-bottom: 16px;
  }
  .patentFacadeAuthorizationPay-box-select > p {
    font-family: "PingFangSC";
    font-size: 20px;
    margin-bottom: 16px;
    color: #333333;
  ...
  .patentFacadeAuthorizationPay-box-select > input {
    width: 18px;
    height: 18px;
  ...
  .patentFacadeAuthorizationPay-box-select > span {
    font-family: "PingFangSC";
    font-size: 20px;
    vertical-align: middle;
    margin-right: 65px;
  ...
  /* 支付方式 */
  .patentFacadeAuthorizationPay-box-way {
    margin: 0px 0 36px 0;
    border-bottom: 1px solid #d8d8d8;
    padding-bottom: 36px;
  }
  .patentFacadeAuthorizationPay-box-way > p {
    font-family: "PingFangSC";
    font-size: 22px;
    font-weight: 600;
  }
  ...
  /* 总金额 */
  .patentFacadeAuthorizationPay-box-money > p:nth-child(1) {
    font-family: "PingFangSC";
    font-size: 22px;
    font-weight: 600;
    color: #333;
  ...
  .patentFacadeAuthorizationPay-box-money > p:nth-child(2) {
    font-family: "PingFangSC";
    font-size: 26px;
    font-weight: 600;
    color: #333;
  }
  .patentFacadeAuthorizationPay-box-money > p:nth-child(2) >span {
    color: #f96006;
  }
  /* 提示 */
  .patentFacadeAuthorizationPay-box-prompt > p {
    font-family: "PingFangSC";
    font-size: 22px;
    font-weight: 600;
    color: #333;
  ...
  .patentFacadeAuthorizationPay-box-prompt > div > a {
    font-family: "PingFangSC";
    font-size: 22px;
    font-weight: 600;
    color: #f96006;
  ...
  /* 提交&&取消 */
  .patentFacadeAuthorizationPay-box-submit {
    float: right;
    margin:64px 0px 0 0 ;
  }
  .patentFacadeAuthorizationPay-box-submit > span {
    width: 214px;
    height: 54px;
  ...
  .patentFacadeAuthorizationPay-box-submit > span:nth-child(2) {
  ...
  .patentFacadeAuthorizationPay-box-submit > span:nth-child(1) {

    border-radius: 4px;
    color: #f87604;
  }
    .patentFacadeAuthorizationPay-box-submit > span:nth-child(3) {
    border-image-source: linear-gradient(0deg,#ba6100,#dc773b);
    border-image-slice: 1;
    background-image: linear-gradient(180deg,#f79000,#f96006),linear-gradient(0deg,#ba6100,#dc773b);
    background-origin: border-box;
