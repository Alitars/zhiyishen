    ...
    <Fix></Fix>
     ...
     <div class="check-box-big">
       <div class="check-box-big-address">
          <div class="address-div1">
            ...
            <div>
                <select v-model="chose" @change="pick">
                  <option selected="true">请选择收件地址</option>
          ...
          <div class="address-div2" @click="addaddress">+收件地址</div>
       </div>
       ...
       <div class="check-box-big-order">
         ...
         <div class="check-box-big-order-list" v-for="(v,k) in paylist">
           ...
           <div v-if="v.one_cate">第{{v.one_cate}}类 {{v.one_cate_name}}</div>
           <div v-else>无</div>
           <div>￥{{v.price}}</div>
         </div>
       </div>
       <div class="check-box-big-discount" v-show="zeroshow">
         ...
         <select name="" id="" v-model="select" @change='getValue'>
           <option>请选择</option>
           <option  v-for="(v,k) in conpon" v-bind:value="v.id">{{v.title}}，优惠金额：{{v.user_total}}元</option>
         </select>
       </div>
...
export default {
  ...
  computed:{
    money(){
      if(this.select == "请选择"){
            this.paytotal = this.total-this.coupon_price;
          }
    }
  },
  methods:{
    ...
    Tonext(){
      ...
      if(this.paytotal ==0){
        ...
        this.$http.post(this.GLOBAL.base_url+'/api/order/offinePayOrder',{
              user_id:this.$cookie.getCookie('u_id'),
              order_id:a,
              c_id:this.select,
              pay_type:0,
              shipp_address_id:this.chose,
              invoice_id:this.pickcompany
            }).then(res=>{
              if(res.data.code == "1"){
                this.tip('success','支付成功')
                // this.$router.push('/personal/order?status=all')
                if(this.paylist[0].good_name == "商标注册"){
                    this.$router.push({
                        path:'/registerData',
                        query:{
                            click_type:this.paylist[0].click_type,
                            order_id:this.paylist[0].order_id,
                            edit:1
                        }
                        });
                }else if(this.paylist[0].click_type == "商标变更"){
                  this.$router.push({
                      path:'/changeData',
                      query:{
                          click_type:this.paylist[0].click_type,
              ...
              }else if(this.paylist[0].click_type == "商标续展"||this.paylist[0].click_type == "商标宽展"){
                  this.$router.push({
                      path:'/extension',
                      query:{
                          click_type:this.paylist[0].click_type,
              ...
              }else if(this.paylist[0].click_type == "商标转让"){
                  this.$router.push({
                      path:'/transferData',
                      query:{
                          click_type:this.paylist[0].click_type,
              ...
              }else if(this.paylist[0].click_type == "商标许可备案"){
                  this.$router.push({
                      path:'/filingData',
                      query:{
                          click_type:this.paylist[0].click_type,
              ...
              }else if(this.paylist[0].click_type == "补发商标证书"){
                  this.$router.push({
                      path:'/resendData',
                      query:{
                          click_type:this.paylist[0].click_type,
              ...
              }else if(this.paylist[0].click_type == "商标驳回复审"){
                  this.$router.push({
                      path:'/recheck',
                      query:{
                          click_type:this.paylist[0].click_type,
              ...
              }else if(this.paylist[0].click_type == "商标异议申请"){
                  this.$router.push({
                      path:'/objectData',
                      query:{
                          click_type:this.paylist[0].click_type,
              ...
              }else if(this.paylist[0].click_type == "商标异议答辩"){
                  this.$router.push({
                      path:'/defence',
                      query:{
                          click_type:this.paylist[0].click_type,
              ...
              }else if(this.paylist[0].click_type == "商标撤三申请"){
                  this.$router.push({
                      path:'/threeApply',
                      query:{
                          click_type:this.paylist[0].click_type,
              ...
              }else if(this.paylist[0].click_type == "商标撤三答辩"){
                  this.$router.push({
                      path:'/threeAnswer',
                      query:{
                          click_type:this.paylist[0].click_type,
      ...
      if(this.active == "company"&&this.paytotal != 0){
        ...
        this.$http.post(this.GLOBAL.base_url+'/api/order/offinePayOrder',{
              ...
              shipp_address_id:this.chose,
              invoice_id:this.pickcompany,
              company_transfer:this.countcompany
            }).then(res=>{
              if(res.data.code == "1"){
       ...
       if(this.active == "wei"&&this.paytotal != 0){
        // console.log(3);
        if(this.select == "请选择"){
              this.select = 0
            }
    ...
    sucess(){
      if(this.pay_order_no != ''){
        this.$http.get(this.GLOBAL.base_url+'/api/order/getorderstatus',{params:{
          ...
          if(this.select == "请选择"){
            ...
            for(var i=0;i<this.conpon.length;i++){
              if(this.select == this.conpon[i].id){
                this.paytotal = this.total - this.conpon[i].user_total-this.coupon_price;
              }
            }
          }
        },
  ...
  mounted(){
    this.$http.post(this.GLOBAL.base_url+'/api/order/checkOrder',{
          ...
          // console.log(res)
            if(res.data.code != "0"){
              ...
              this.conpon = res.data.nouse_conpon;
