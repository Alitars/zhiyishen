    ...
    <div class="patentDetali">
      ...
      <div class="patentDetail-crumbs">
        <el-breadcrumb class="bread-card-crumb" separator-class="el-icon-arrow-right">
          ...
          <el-breadcrumb-item :to="{path:'/patent',query: {id: 49}}">专利服务</el-breadcrumb-item>
...
export default {
  ...
  methods: {
    ...
    addCart(){
      if ((Date.now() - this.onpresscTime) > 2000) {
        ...
        if(this.$cookie.getCookie('u_id')){
          this.$http.post(this.GLOBAL.base_url+'/api/patent/addCart',{
            user_id:this.$cookie.getCookie('u_id'),
            good_id:this.typeId,
    ...
    tobuy() {
      if(this.value1 != '1') {
        ...
        this.$http.post(this.GLOBAL.base_url +'/api/patent/addOrder',{
          user_id:this.$cookie.getCookie('u_id'),
          click_type:this.type_title,
          id:this.typeId
        }).then( res => {
  ...
  mounted(){
    this.getData()
    // if(this.$route.query.secondid == 85) {
    //   this.analysis_show = true
    // }else {
    //   this.analysis_show = false
    // }
