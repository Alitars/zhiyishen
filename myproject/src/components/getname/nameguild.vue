<template>
  <div class="nameguild">
    <Top></Top>
    <Fix></Fix>
     <div class="nameguild-box">
       <div v-show="false" ref="div1">请输入您的产品或服务名称，并选择所属行业：</div>
       <div class="nameguild-box-top">
       ...
       <div class="nameguild-box-container">
         <div v-show="boxshow">
            <div class="nameguild-box-get">
              <div v-for="(m,n) in 7">
                <div v-for="(v,k) in [...trade].splice((m-1)*3,3)" @click="find(v.id,m)" class="nameguild-box-get-big" :class='{active:v.id==id}'>
                  <span >{{v.title}}</span>   
                </div>
                <div class="nameguild-box-get-little">
                    <div v-show="num1 == m"  v-for="(item,index) in little">
                      <input type="checkbox" v-model="box3" :value="item.groups" checked="pick(item.groups)">
...
export default {
  ...
  methods:{
    ...
    setTimer(){
        ...
        togood(){
          if(this.box3.length == 0 || this.key == ''){
        ...
        pick(val){
          if(this.box3.length == 0){
            ...
            for(var i = 0;i<this.box3.length;i++){
            if(val == this.box3[i]){
              return true;
            }else{
              return false
...
<style scoped>
  ...
  .nameguild-box-top>img{
    float: left;
  }
  .nameguild-box-top>div{
    float: left;
    width: 680px;
  ...
  .nameguild-box-container{
    height: 832px;
  }
  .nameguild-box-get-big{
    float: left;
    border-radius: 4px;
  ...
  .nameguild-box-get-big:nth-child(3n){
    margin-right: 0;
  }
  .active{
  ...
  .nameguild-box-get-little{
    width: 947px;
    margin-bottom: 20px;
    box-shadow: 0 0 30px -1px rgba(217, 217, 217, 0.5);
  }
  .nameguild-box-get-little>div{
    float: left;
    margin: 10px 0 10px 20px;
  }
  .nameguild-box-get-little>div>input{
    vertical-align: middle;
    cursor: pointer;
  }
  .nameguild-box-get-little>div>span{
    vertical-align: middle;
    font-size: 16px;
  }
  .nameguild-box-next{
  ...
  .nameguild-box-next>div:nth-of-type(1){
       margin-right: 52px;
       background-color: #efefef;
       color: #f87e03;
  }
  .nameguild-box-next>div:nth-of-type(2){
       background-color: #f96006;
       color: #ffffff;
  }
   .input-div{
