...
Page({
      ...
      func.onTrade(text).then(res => {
        if (res.data.code == 1) {
          if (res.data.data.is_empty == 0) {
            ...
            wx.redirectTo({
              url: '/package/component1/pages/index/check_history/check_history?text=' + text + '&key=' + key,
            })
          } else {
            getApp().globalData.tradeArry = res.data.data.data;
            wx.navigateTo({
              url: '/package/component1/pages/index/choose_history/choose_history?text=' + text + '&key=' + key,
            })
          }
        }
      }).catch(err => {})
  ...
  onLoad: function(options) {
    ...
    vioceText({
      data: {
        content: 'Hi!我是您的AI知识产权代理人--小果,请输入您的产品名称或服务行业',
      },
    }).then(res => {})
  },
  onUnload: function() {
