...
Page({
  data: {
    ...
    history: null,
    popup: [{
      icon: '/package/component1/images/pp1.png',
      itxt: '专业',
      txt: '人工智能大数据精确比对智能算法精准分析审查规则'
    }, {
      icon: '/package/component1/images/pp2.png',
      itxt: '省心',
      txt: '小果AI极速为您商标取名，Logo设计，在线撰写技术交底书'
    }, {
      icon: '/package/component1/images/pp3.png',
      itxt: '保障',
  ...
  onType(e) {
    ...
    var total = (typeof (arry[type].service_price) == 'string' ? JSON.parse(arry[type].service_price) : arry[type].service_price) + (typeof (arry[type].price) == 'string' ? JSON.parse(arry[type].price) : arry[type].price);
    this.setData({
      type: type,
    ...
    NetworkRequest({
      ...
      if (arry) {
        for (var i = 0; i < arry.length; i++) {
          // console.log();
          arry[i].total = (typeof (arry[i].service_price) == 'string' ? JSON.parse(arry[i].service_price) : arry[i].service_price) + (typeof (arry[i].price) == 'string' ? JSON.parse(arry[i].price) : arry[i].price);
        ...
        var total = JSON.parse(arry[0].service_price) + JSON.parse(arry[0].price);
        if (this.data.key.length > 0) {
          for (var i = 0; i < arry.length; i++) {
            if (arry[i].title == this.data.key) {
              var type = i;
              var total = JSON.parse(arry[type].service_price) + JSON.parse(arry[type].price);
              this.setData({
                type: type,
                total: total.toFixed(2),
              });
  ...
  onLoad: function (options) {
    ...
    if (options.open=='true') {
      this.data.key = options.key||wx.getStorageSync('title');
    }else{
      this.data.key = wx.getStorageSync('title');
    };
  },
