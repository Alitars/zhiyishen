...
Page({
  ...
  onBack() {
    ...
    for (var i = 0; i < pages.length; i++) {
      if (pages[i].route == 'package/component1/pages/intitle/bename/bename') {
        wx.navigateBack({
          delta: i + 1,
        });
        break;
  ...
  onDt(e) {
    ...
    var nameArry = JSON.parse(JSON.stringify(getApp().globalData.nameArry));
    var imgUrlsIndex = nameArry[0].fir;
    var len = nameArry.length;
    for (var i = 0; i < len; i++) {
      nameArry[i].fir = imgUrlsIndex;
    };
    for (var n = 0; n < len; n++) {
    ...
    wx.navigateTo({
      url: '/package/component1/pages/intitle/createDt_history/createDt_history?index=' + index,
    });
  },
  onRecord(arry) {
    var name = this.data.name;
    var imgUrlsIndex = JSON.parse(JSON.stringify(getApp().globalData.nameArry[0].fir));
    var record = wx.getStorageSync('applyType');
    for (var i = 0; i < arry.length; i++) {
      arry[i].fir = imgUrlsIndex;
