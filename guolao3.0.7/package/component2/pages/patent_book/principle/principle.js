...
Page({
  ...
  onClick() {
    ...
    list.ans3 = this.data.value
    var json = JSON.stringify(list)
    wx.navigateTo({
      // url: '/package/component2/pages/patent_book/antistop/antistop?compose='+this.data.compose+'&list='+ json,
      url:'/package/component2/pages/patent_book/upload_photo/upload_photo?compose='+this.data.compose+'&list='+ json,
    })
  },
  onSlider(e) {
  ...
  onLoad: function (options) {
    ...
    vioceText({
      data: {
        content: '通过集成电路布图设计描述其层级设计以及层级间的连接关系；描述其工作原理及实现方式；如需帮助请点击示例查看',
      },
    })
  },
